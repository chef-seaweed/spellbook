...
(
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('pancakeswap_v2_zksync', 'PancakePair_evt_Swap') }} t
    INNER JOIN
        {{ source('pancakeswap_v2_zksync', 'PancakeFactory_evt_PairCreated') }} f
...
SELECT
    'zksync'                                                   AS blockchain
     ...
     , dexs.evt_index
FROM dexs
INNER JOIN {{ source('zksync', 'transactions') }} tx
...
LEFT JOIN {{ ref('tokens_erc20') }} erc20a
    ...
    AND erc20a.blockchain = 'zksync'
LEFT JOIN {{ ref('tokens_erc20') }} erc20b
    ON erc20b.contract_address = dexs.token_sold_address
    AND erc20b.blockchain = 'zksync'
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ON p_bought.minute = date_trunc('minute', dexs.block_time)
    AND p_bought.contract_address = dexs.token_bought_address
    AND p_bought.blockchain = 'zksync'
    {% if not is_incremental() %}
